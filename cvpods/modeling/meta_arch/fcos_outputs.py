...
# from cvpods.layers.nms import ml_nms
from cvpods.layers import IOULoss
...
class FCOSOutputs(nn.Module):
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, locations, gt_instances, top_feats=None,branch_name=""):
        """
        ...
        return self.fcos_losses(instances,branch_name=branch_name)

    def fcos_losses(self, instances,branch_name=""):
        losses, extras = {}, {}
        ...
        losses["loss_fcos_cls"+branch_name] = class_loss * self.loss_weight_cls
        ...
        if self.box_quality == "ctrness":
            ...
            extras["loss_denorm"+branch_name] = loss_denorm

            reg_loss = self.loc_loss_func(ious, gious, ctrness_targets) / loss_denorm
            losses["loss_fcos_loc"+branch_name] = reg_loss

            ctrness_loss = F.binary_cross_entropy_with_logits(
                instances.ctrness_pred, ctrness_targets,
                reduction="sum"
            ) / num_pos_avg
            losses["loss_fcos_ctr"+branch_name] = ctrness_loss
            ...
            losses["loss_fcos_iou"+branch_name] = quality_loss
        else:
    ...
    def predict_proposals(
            ...
            locations, image_sizes, top_feats=None,counting=False
    ):
        if self.training or counting:
