...
// modified from
// https://github.com/facebookresearch/detectron2/blob/main/detectron2/layers/csrc/cocoeval/cocoeval.cpp
    ...
    for (auto d = 0; d < num_detections; ++d) {
      ...
      detection_ignores[t * num_detections + d] =
          ...
          (detection_matches[t * num_detections + d] == 0 &&
           (detection.area < area_range[0] || detection.area > area_range[1] || detection.is_neg));
    }
  }
  ...
  for (auto c = 0; c < num_categories; ++c) {
    for (auto a = 0; a < num_area_ranges; ++a) {
      for (auto m = 0; m < num_max_detections; ++m) {
        ...
        // LVISeval::EvaluateImages() is one long list storing results for each
