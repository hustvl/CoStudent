...
struct InstanceAnnotation {
  InstanceAnnotation(
      ...
      bool is_crowd,
      bool ignore)
      : id{id}, score{score}, area{area}, is_neg{is_neg}, is_crowd{is_crowd}, ignore{ignore} {}
  uint64_t id;
  double score = 0.;
  double area = 0.;
  bool is_neg = false;
...
// C++ implementation of COCO API cocoeval.py::LVISeval.evaluateImg().  For each
...
// interpreted by the LVISeval::Accumulate() function to produce precion-recall
...
// C++ implementation of LVISeval.accumulate(), which generates precision
...
// evaluations is the return value of the functon LVISeval::EvaluateImages(),
