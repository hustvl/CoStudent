...
class COCODataset(BaseDataset):
    def __init__(self, cfg, dataset_name, transforms=None, is_train=True):
        ...
        if is_train:
            self.one_brance_train = cfg.INPUT.get('ONE_BRANCE_TRAINING')
        else:
            self.one_brance_train = False
        ...
        if "panoptic" in dataset_name:
            ...
            self.task_key = "voc"           # for coco-type voc datasets
        else:
            self.task_key = "coco"          # for task: instance detection/segmentation
        self.merge_voc = False
        self.meta = self._get_metadata(dataset_name)
        if self.task_key in ["coco", "coco_person","voc"]:
    ...
    """)
        ...
        for (img_dict, anno_dict_list) in imgs_anns:
            record = {}
            if not self.merge_voc:
                record["file_name"] = os.path.join(image_root,
                                               img_dict["file_name"]) 
            else:
                year = '2007' if len(img_dict["file_name"].split('_'))==1 else '2012'
                image_root = osp.join(osp.dirname(image_root),'VOC'+year)
                record["file_name"] = os.path.join(image_root,"JPEGImages",
                                               img_dict["file_name"]) 
    ...
    def _get_metadata(self,dataset_name):
        if self.task_key in ["coco", "coco_person"]:
            ...
            meta["json_file"] = osp.join(data_root, json_file) \
        ...
        elif self.task_key in ["voc"]:

            from .paths_route import _PREDEFINED_SPLITS_VOC
            self.year = dataset_name.split('_')[2]
            if self.year == "0712":
                self.merge_voc = True
            image_root, split = osp.join('VOCdevkit' ,'VOC'+str(self.year)) , dataset_name.split('_')[-1]
            ...
            if 'missing50p' in dataset_name:
                json_file = "/path/pascal_"+self.split+self.year+"_missing50p.json"
            else:
                json_file = "/path/pascal_"+self.split+self.year+".json"
            meta = {
