...
class SimpleRunner(RunnerBase):
    ...
    def _write_metrics(
        ...
        all_custom_metrics = custom_metrics
        ...
        with torch.cuda.stream(torch.cuda.Stream() if device.type == "cuda" else None):
            ...
            if custom_metrics is not None:
                if isinstance(custom_metrics['found_num'],list):
                    custom_metrics_1 = dict()
                    custom_metrics_2 = dict()
                    custom_metrics_3 = dict()
                    for key,value in custom_metrics.items():
                        custom_metrics_1[key+'_1'] = value[0]
                        custom_metrics_2[key+'_2'] = value[1]
                        custom_metrics_3[key+'_3'] = value[2]
                    all_custom_metrics_1 = comm.gather(custom_metrics_1)
                    all_custom_metrics_2 = comm.gather(custom_metrics_2)
                    all_custom_metrics_3 = comm.gather(custom_metrics_3)
                    all_custom_metrics = [all_custom_metrics_1,all_custom_metrics_2,all_custom_metrics_3]
                else:
                    custom_metrics_1 = dict()
                    for key,value in custom_metrics.items():
                        custom_metrics_1[key+'_1'] = value
                    all_custom_metrics = [comm.gather(custom_metrics_1)]
        ...
        if comm.is_main_process():
            ...
            if custom_metrics is not None:
                ...
                for i,all_cm in enumerate(all_custom_metrics):
                    custom_metrics_sum = {
                    k: np.sum([x[k] for x in all_cm]) for k in all_cm[0].keys()
                }   

                    custom_metrics_sum['missAR_{}'.format(i)] =  (custom_metrics_sum['found_num_{}'.format(i+1)] + \
                                        custom_metrics_sum['ori_num_{}'.format(i+1)]) / custom_metrics_sum['all_num_{}'.format(i+1)]
                    storage.put_scalars(**custom_metrics_sum)

                    # if pbar is not None:
                    #     if i == 0:
                    #         postfix = OrderedDict([
                    #         ('total', custom_metrics_sum['all_num_1']),
                    #         ('ori',custom_metrics_sum['ori_num_1']),
                    #         ('found_{}'.format(i), custom_metrics_sum['found_num_1']),
                    #         ('wrong_loc_{}'.format(i), custom_metrics_sum['wrong_loc_num_1']),
                    #         ('wrong_cls_{}'.format(i),custom_metrics_sum['wrong_cls_num_1']),
                    #         ('missAR_{}'.format(i), custom_metrics_sum['missAR_{}'.format(i)] )
                    #         ])
                    #     else:
                    #         postfix.update([
                    #         ('found_{}'.format(i), custom_metrics_sum['found_num_2']),
                    #         ('wrong_loc_{}'.format(i), custom_metrics_sum['wrong_loc_num_2']),
                    #         ('wrong_cls_{}'.format(i),custom_metrics_sum['wrong_cls_num_2']),
                    #         ('missAR_{}'.format(i), custom_metrics_sum['missAR_{}'.format(i)])
                    #         ])
