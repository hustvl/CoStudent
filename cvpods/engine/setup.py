...
def default_argument_parser():
    ...
    parser.add_argument(
        "--gpus-temp-monitor",
        action="store_true",
        help="whether to employ a monitor to supervise its temperature",
    )
...
def adjust_config(cfg):
    ...
    # learning rate in original config is used for 8 GPUs

    world_size = comm.get_world_size()
    total_batch = cfg.SOLVER.IMS_PER_DEVICE * world_size
    # cfg.SOLVER.IMS_PER_BATCH = total_batch

    # assert cfg.SOLVER.LR_SCHEDULER.MAX_ITER == 1440000, "Only training for 1x. (1440000 img_iter)"

    cfg.SOLVER.LR_SCHEDULER.MAX_ITER = int(cfg.SOLVER.LR_SCHEDULER.MAX_ITER / total_batch)

    cfg.SOLVER.LR_SCHEDULER.STEPS = tuple([int(x/total_batch) for x in cfg.SOLVER.LR_SCHEDULER.STEPS])

    cfg.SOLVER.CHECKPOINT_PERIOD = int(cfg.SOLVER.CHECKPOINT_PERIOD / total_batch)

    START_SPLG_ITER = cfg.TRAINER.get('START_SPLG_ITER')
    if START_SPLG_ITER is not None:
        cfg.TRAINER.START_SPLG_ITER = int(cfg.TRAINER.START_SPLG_ITER / total_batch)

    cfg.TEST.EVAL_PERIOD = int(cfg.TEST.EVAL_PERIOD / total_batch)

    cfg.SOLVER.OPTIMIZER.BASE_LR = cfg.SOLVER.OPTIMIZER.BASE_LR / 16 * total_batch
    
    logger.info("learning rate:{}.".format(cfg.SOLVER.OPTIMIZER.BASE_LR))
...
def default_setup(cfg, args):
    ...
    logger_name = cfg.get('LOGGER_NAME',"log.txt")
    if comm.is_main_process() and output_dir:
        ensure_dir(output_dir)

    rank = comm.get_rank()
    # setup_logger(output_dir, distributed_rank=rank, name="cvpods")
    setup_logger(output_dir, distributed_rank=rank,logger_name=logger_name)
